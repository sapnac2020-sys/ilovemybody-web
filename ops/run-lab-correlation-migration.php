...
$view=(int)$pdo->query("SELECT COUNT(*) FROM v_ilmb_loinc_chebi_compute")->fetchColumn();
$rule=(int)$pdo->query("SELECT COUNT(*) FROM ilb_lab_correlation_rule WHERE rule_key='lab_reference_position' AND version_label='1.0' AND status='active'")->fetchColumn();
$out=['crosswalk'=>$row,'view_rows'=>$view,'active_rules'=>$rule,'patient_rows_read'=>0,'patient_rows_modified'=>0,'ready'=>$view>0&&$rule===1&&(int)$row['eligible']>0];

...
$config=$argv[1]??''; $sql=$argv[2]??''; if(!is_file($config)||!is_file($sql)) exit(2);
...
$st=$pdo->prepare((string)file_get_contents($sql));$st->execute();do{if($st->columnCount())$st->fetchAll();}while($st->nextRowset());$st->closeCursor();
...
echo json_encode($out,JSON_PRETTY_PRINT),PHP_EOL;if(!$out['ready'])exit(4);

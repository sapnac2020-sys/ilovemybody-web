...
$q=$pdo->query("SELECT system_id,sheet_name,COUNT(*) rows_count FROM ilmb_canonical_record WHERE active=1 AND (UPPER(system_id) LIKE '%CHEBI%' OR UPPER(sheet_name) LIKE '%CHEBI%' OR sheet_name IN ('Entities','Ontology_Identity_Stubs','Chemical_Data','Core_Structures')) GROUP BY system_id,sheet_name ORDER BY rows_count DESC");$out['chebi_canonical_locations']=$q->fetchAll();
$q=$pdo->prepare("SELECT table_name,table_type FROM information_schema.tables WHERE table_schema=? AND LOWER(table_name) LIKE '%chebi%' ORDER BY table_name");$q->execute([$db]);$out['chebi_database_objects']=$q->fetchAll();
$q=$pdo->query("SELECT system_id,file_name,status,row_count,promoted_at FROM ilmb_sync_batch WHERE UPPER(system_id) LIKE '%CHEBI%' OR UPPER(file_name) LIKE '%CHEBI%' ORDER BY created_at DESC LIMIT 100");$out['chebi_batches']=$q->fetchAll();

CREATE TABLE IF NOT EXISTS ilmb_parameter_master (
 ...
 parameter_key VARCHAR(191) NOT NULL UNIQUE,
 ...
 KEY ix_ilmb_parameter_domain (parameter_domain), KEY ix_ilmb_parameter_name (canonical_name)
...
CREATE TABLE IF NOT EXISTS ilmb_parameter_identifier (
 ...
 source_url VARCHAR(1024) NULL, source_version VARCHAR(64) NULL,
 verified_at DATETIME NULL, verified_by VARCHAR(191) NULL,
 created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS ilmb_formula_master (
 ...
 formula_key VARCHAR(191) NOT NULL UNIQUE,
 ...
 source_citation TEXT NULL, source_url VARCHAR(1024) NULL, source_version VARCHAR(64) NULL,
...
CREATE TABLE IF NOT EXISTS ilmb_parameter_duplicate_candidate (
 ...
 confidence DECIMAL(6,5) NULL, evidence_json JSON NULL,
 disposition ENUM('OPEN','SAME','DISTINCT','MERGED','IGNORED') NOT NULL DEFAULT 'OPEN',
 reviewed_at DATETIME NULL, reviewed_by VARCHAR(191) NULL,
 created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS ilmb_body_need_run (
 ...
 blocking_reason TEXT NULL, provenance_json JSON NULL,
 created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
FROM ilmb_parameter_master p LEFT JOIN ilmb_parameter_identifier i ON i.parameter_id=p.parameter_id
WHERE p.status<>'DEPRECATED' GROUP BY p.parameter_id;

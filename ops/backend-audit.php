...
$identifierCoverage = [];
$identifierColumnsStmt = $pdo->prepare(
...
foreach ($identifierColumnsStmt->fetchAll() as $row) {
    ...
    $coverage = $pdo->query(
        "SELECT COUNT(*) AS total_rows,
                ...
                COUNT(DISTINCT CASE WHEN {$quotedColumn} IS NOT NULL
                                     AND CAST({$quotedColumn} AS CHAR) <> ''
                                    THEN {$quotedColumn} END) AS distinct_values
           FROM {$quotedTable}"
    )->fetch();
    $identifierCoverage[] = [
        'table_name' => $table,

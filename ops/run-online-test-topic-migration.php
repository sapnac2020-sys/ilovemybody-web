...
$required=(int)$pdo->query("SELECT COUNT(*) FROM ilb_test_topic_catalog WHERE topic_key IN ('blood_count_iron','glucose_metabolism','heart_electrical_imaging','brain_neurological_imaging','skin_hair','preventive_screening') AND status='active'")->fetchColumn();
$out=['active_topics'=>$count,'required_topics'=>$required,'patient_rows_read'=>0,'patient_rows_modified'=>0,'ready'=>$count===18&&$required===6];

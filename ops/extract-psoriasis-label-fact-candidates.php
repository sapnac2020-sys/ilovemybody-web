...
function context(string $text,int $offset,int $length):string{$start=max(0,$offset-180);$span=mb_substr($text,$start,$length+360,'UTF-8');return trim((string)preg_replace('/\s+/u',' ',$span));}
$pattern='/\b(\d+(?:\.\d+)?)\s*(mg\/kg|mg|mcg|µg|ug|ng\/mL|mcg\/mL|µg\/mL|ug\/mL|mg\/mL|mL|L|%|hours?|days?|weeks?|months?|years?)\b/iu';
$candidates=[];
...
$covered=array_values(array_unique(array_column($candidates,'agent')));if(count($covered)!==11)throw new RuntimeException('Numeric candidates do not cover all 11 agents: '.json_encode($covered));
...
echo json_encode(['database'=>$database,'phase'=>'P82_LABEL_FACT_CANDIDATES','source_sections_scanned'=>count($rows),'covered_agents'=>count($covered),'candidate_count'=>count($candidates),'by_kind'=>$byKind,'readiness'=>$readiness,'computation_eligible_candidates'=>$eligible,'patient_tables_queried'=>false,'patient_rows_read'=>0,'patient_rows_modified'=>0,'verified'=>true],JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES),PHP_EOL;

...
function textCell(SimpleXMLElement $c, array $shared): string { $a=$c->attributes();$t=(string)($a['t']??'');if($t==='s')return $shared[(int)$c->v]??'';if($t==='inlineStr')return (string)$c->is->t;return (string)$c->v; }
function readXlsx(string $path): array {
    ...
    $shared=[]; if(($s=$z->getFromName('xl/sharedStrings.xml'))!==false){$x=simplexml_load_string($s);foreach($x->si as $si){$parts=$si->xpath('.//t');$shared[]=implode('',array_map('strval',$parts));}}

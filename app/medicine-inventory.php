...
try {
  ...
  foreach($sheet as $s){$q->execute([$s['system_id'],$s['sheet_name']]);$rows=[];foreach($q->fetchAll() as $r){$p=json_decode((string)$r['payload_json'],true);$rows[]=['source_key'=>$r['source_key'],'fields'=>is_array($p)?array_slice($p,0,20,true):[]];}$samples[]=['system_id'=>$s['system_id'],'sheet_name'=>$s['sheet_name'],'rows'=>$rows];}
  ...
  echo json_encode(['ok'=>true,'mode'=>'read_only_medicine_inventory','patient_data_read'=>false,'patient_data_written'=>false,'canonical_sheets'=>$sheet,'crosswalk_groups'=>$x,'matching_tables'=>$tables,'samples'=>$samples],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);

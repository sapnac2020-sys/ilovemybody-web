...
try{
 ...
 $provided=array_key_exists('value',$_GET)||array_key_exists('low',$_GET)||array_key_exists('high',$_GET)||array_key_exists('unit',$_GET)||array_key_exists('reference_unit',$_GET);
 $calc=['status'=>'not_requested','classification'=>null,'reason'=>'Supply value, low, high, unit and reference_unit to run the governed comparison.'];
 if($provided){
  ...
  if(!is_numeric($raw['value'])||!is_numeric($raw['low'])||!is_numeric($raw['high'])) $calc=['status'=>'blocked','classification'=>null,'reason'=>'Value and interval boundaries must all be numeric.'];
  elseif($unit===''||$referenceUnit===''||$unit!==$referenceUnit) $calc=['status'=>'blocked','classification'=>null,'reason'=>'Result and reference units must be present and exactly identical; no implicit conversion was used.'];
  ...
  else{$v=(float)$raw['value'];$lo=(float)$raw['low'];$hi=(float)$raw['high'];$class=$v<$lo?'below':($v>$hi?'above':'within');$calc=['status'=>'calculated','classification'=>$class,'reason'=>'Compared only with the supplied interval.','inputs'=>['value'=>$v,'low'=>$lo,'high'=>$hi,'unit'=>$unit]];}
 }
 $chain=[['layer'=>'measurement_identity','system'=>'LOINC','id'=>$loinc,'label'=>$identity['loinc_name']],['layer'=>'approved_crosswalk','gate'=>'EXACT + APPROVED + both endpoints resolved','eligible_mappings'=>count($maps)],['layer'=>'chemical_identity','system'=>'ChEBI','entities'=>array_map(fn($m)=>['id'=>$m['chebi_id'],'name'=>$m['chebi_name'],'predicate'=>$m['predicate'],'evidence_source'=>$m['evidence_source'],'evidence_version'=>$m['evidence_version'],'evidence_locator'=>$m['evidence_locator'],'confidence'=>$m['confidence'],'loinc_release'=>$m['loinc_release'],'chebi_release'=>$m['chebi_release']],$maps)],['layer'=>'unit_gate','rule'=>'Case-sensitive exact unit equality; no invented conversion.'],['layer'=>'calculation','rule'=>$rule,'result'=>$calc]];
 respond(200,['ok'=>true,'mode'=>'non_patient_golden_proof','patient_data_read'=>false,'patient_data_written'=>false,'loinc'=>array_merge(['code'=>$loinc],$identity),'chain'=>$chain,'calculation'=>$calc,'limitations'=>['No diagnosis or treatment recommendation.','A supplied interval is not assumed universal.','Only computation-eligible exact approved mappings are returned.','No patient record is stored by this endpoint.']]);
